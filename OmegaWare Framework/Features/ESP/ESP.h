...
private:
	...
	CG::UClass* PlayerStaticClass = nullptr;

	// Hostile
	CG::UClass* ZombieStaticClass = nullptr;
...
private:
	...
	void DrawBox(CG::AActor* Actor, std::string Name, ImVec2 TextSize, std::string Size, int Distance)
	{
		Unreal* pUnreal = Cheat::unreal.get();
		if (!pUnreal)
			return;
	...
	void DrawDebugBox(CG::AActor* Actor, int Distance)
	{
		...
		if (Actor->IsA(ZombieStaticClass))
			Mesh = reinterpret_cast<CG::ADIAICharacter*>(Actor)->Mesh;
	...
	void ZombieESP(CG::ADIAICharacter* Zombie);
};